...
var (
	awsAccountIDRegexp = regexp.MustCompile(`^\d{12}$`)
	digitsRegexp       = regexp.MustCompile(`^\d+$`)
)

func validateVPCEndpointAccess(r *resource) error {
	if err := validatePrincipal(aws.ToString(r.ko.Spec.Principal)); err != nil {
		return err
	}
	if supportedRegionsDeclared(r.ko.Spec.ServiceOptions) && len(supportedRegionSet(r.ko.Spec.ServiceOptions)) == 0 {
		return ackerr.NewTerminalError(
			errors.New("serviceOptions.supportedRegions must include at least one region"),
		)
	}
	return nil
}
...
func validatePrincipal(principal string) error {
	if strings.HasPrefix(principal, "arn:") {
		return ackerr.NewTerminalError(
			errors.New("principal must be a 12-digit account ID or a service principal, not an ARN"),
		)
	}
	if digitsRegexp.MatchString(principal) && !awsAccountIDRegexp.MatchString(principal) {
		return ackerr.NewTerminalError(
			errors.New("an account principal must be a 12-digit account ID"),
		)
	}
	return nil
}
