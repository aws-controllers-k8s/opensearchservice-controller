...
func TestValidatePrincipal(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "twelve-digit account ID is valid",
			principal: "123456789012",
		},
		{
			name:      "service principal is valid",
			principal: "application.opensearchservice.amazonaws.com",
		},
		{
			name:      "ARN is rejected",
			principal: "arn:aws:iam::123456789012:root",
			wantErr:   true,
		},
		{
			name:      "eleven digits is rejected",
			principal: "12345678901",
			wantErr:   true,
		},
		{
			name:      "thirteen digits is rejected",
			principal: "1234567890123",
			wantErr:   true,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if (err != nil) != test.wantErr {
				t.Errorf("validatePrincipal(%q) error = %v, wantErr %v", test.principal, err, test.wantErr)
			}
		})
	}
}
