...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.Principal = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["domainName"]
	if f0ok {
		r.ko.Spec.DomainName = aws.String(f0)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["principal"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: principal"))
	}
	r.ko.Spec.Principal = &primaryKey
	f0, ok := fields["domainName"]
	if !ok {
