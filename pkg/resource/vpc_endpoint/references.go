...
func (rm *resourceManager) EnsureReferences(
	...
	if desiredKO.Spec.VPCOptions != nil {
		if len(desiredKO.Spec.VPCOptions.SecurityGroupRefs) > 0 {
			if latestKO.Spec.VPCOptions == nil {
				latestKO.Spec.VPCOptions = &svcapitypes.VPCOptions{}
			}
			if len(latestKO.Spec.VPCOptions.SecurityGroupRefs) == 0 {
				latestKO.Spec.VPCOptions.SecurityGroupRefs = desiredKO.Spec.VPCOptions.SecurityGroupRefs
			}
		}
		if len(desiredKO.Spec.VPCOptions.SubnetRefs) > 0 {
			if latestKO.Spec.VPCOptions == nil {
				latestKO.Spec.VPCOptions = &svcapitypes.VPCOptions{}
			}
			if len(latestKO.Spec.VPCOptions.SubnetRefs) == 0 {
				latestKO.Spec.VPCOptions.SubnetRefs = desiredKO.Spec.VPCOptions.SubnetRefs
			}
		}
	}
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForDomainARN(ctx, apiReader, ko); err != nil {
...
func validateReferenceFields(ko *svcapitypes.VPCEndpoint) error {
	...
	if ko.Spec.DomainRef != nil && ko.Spec.DomainARN != nil {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("DomainARN", "DomainRef")
	}
	if ko.Spec.DomainRef == nil && ko.Spec.DomainARN == nil {
		return ackerr.ResourceReferenceOrIDRequiredFor("DomainARN", "DomainRef")
	}
...
// resolveReferenceForDomainARN reads the resource referenced
// from DomainRef field and sets the DomainARN
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForDomainARN(
	ctx context.Context,
	apiReader client.Reader,
	...
	if ko.Spec.DomainRef != nil && ko.Spec.DomainRef.From != nil {
		...
		ko.Spec.DomainARN = (*string)(obj.Status.ACKResourceMetadata.ARN)
	}
